...
$(document).ready(function () {
  ...
  scope.find(".dropdown-btn").on("click", function (e) {
    e.preventDefault();
    e.stopPropagation();
    console.log("Botão do dropdown clicado!");

    var $dropdownContent = scope.find(".dropdown-content");
    var $icon = $(this).find("i");
    $dropdownContent.toggleClass("show");
    console.log(
      "Classe 'show' alternada. Dropdown está visível? ",
  ...
  $(document).on("click", function (e) {
    if (!$(e.target).closest("#pmj-container").length) return;

    if (!$(e.target).closest(".dropdown-container").length) {
      const $dropdownContent = scope.find(".dropdown-content");
      if ($dropdownContent.hasClass("show")) {
        ...
        scope.find(".dropdown-btn").css("border-radius", "10px");
      }
    }
  });
  scope.find(".dropdown-item").on("click", function () {
    ...
    scope.find(".selected-text").text(selectedText);
    scope.find(".dropdown-content").removeClass("show");
    scope
      .find(".dropdown-btn i")
      .removeClass("fa-chevron-up")
      .addClass("fa-chevron-down");
    ...
    scope.find("#pmj-container form").hide();
    scope.find(`#${formType}-form`).fadeIn(300);
  });

  scope.find("#btn-sim").on("click", function () {
    console.log("Botão 'Sim' do modal clicado.");
    scope.find("#confirmation-modal").removeClass("show");
    setTimeout(function () {
      scope.find("#confirmation-modal").css("display", "none");
    }, 300);

    scope.find("#pmj-container form").each(function () {
      this.reset();
    ...
    scope.find(".dropdown-container").show();
    scope.find(".selected-text").text("Selecione um tipo de requerimento   ");
  });

  scope.find("#btn-nao").on("click", function () {
    ...
    try {
      ...
      alert(
        "Não foi possível redirecionar. Verifique o console para mais detalhes.",
      );
    }
  });

  scope.find(".enviar-btn").click(function () {
...
`;
    document.getElementById("pmj-container").appendChild(loadingOverlay);
    ...
    setTimeout(function () {
      $.post("/post", {
        ...
        .done(function () {
          ...
          scope.find("#confirmation-modal").addClass("show");
          scope.find("#confirmation-modal").css({
            display: "flex",
          });
  ...
  const BBCodeTemplates = {
    header:
      '[font=Poppins][center][table style="border-color: black; border-radius: 10px; overflow: hidden; width: auto;" bgcolor="#4d4c4c"][tr][td][size=16][color=#ffffff][b]{{titulo}}[/b][/color][/center][/size][/font][/td][/tr][/table]',

    content:
      "[center][size=14][font=Poppins][b][color=black]{{nome}}[/color][/b]{{campos}}[/center][/font][/size]",
    fields: {
